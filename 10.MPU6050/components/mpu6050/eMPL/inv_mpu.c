...
//返回值:0,正常
//    其他,失败
u8 mpu_dmp_init(mpu6050_config_t *cfg)
{
	u8 res=0;
	MPU_Init(cfg); 	//初始化IIC总线
...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
	...
	unsigned char more;
	long quat[4]; 
	while(dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors,&more) && count--){};
    if( count == 0){
        ESP_LOGE("mpu_dmp_get_data", "error in read fifo");
        return 1;
    }	 
